...
export default {
  ...
  methods: {
    ...
    async save() {
      ...
      if (this.id) {
        //修改
        res = await this.$vueHttp.put(`rest/heroes/${this.id}`, this.model);
      } else {
        //新增
        res = await this.$vueHttp.post("rest/heroes", this.model);
      }
    ...
    async fetch() {
      const res = await this.$vueHttp.get(`rest/heroes/${this.id}`);
    ...
    async fetchItems() {
      const res = await this.$vueHttp.get(`rest/items`);
      this.items = res.data;
    }
  },
  created() {
